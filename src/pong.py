...
s_width = 800
s_height = 600
play_width = 600  # meaning 600 // 20 = 20 width per block
...
def create_grid(locked_positions={}):
    grid = [[black for x in range(20)] for x in range(10)]
...
def valid_space(sand, grid):
    accepted_positions = [[(j, i) for j in range(20) if grid[i][j] == black] for i in range(10)]
    accepted_positions = [j for sub in accepted_positions for j in sub]
    formatted = convert_shape_format(sand)
...
def get_shape():
    global sand, sand_color
    return Piece(11, 0, sand)
